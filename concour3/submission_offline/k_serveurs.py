...
def offline_k_serveurs(pos_sites, k, demandes):
    ...
    for d in range(len(demandes)):
        G.add_node("r_" + str(d))
        G.add_node("r1_" + str(d))

    # Create the graph's edges
    for s in range(k):
        G.add_edge("s", "s_" + str(s), capacity=1, weight=0)

    for s in range(k):
        for d in range(len(demandes)):
            position_site = pos_sites[demandes[d]]

            distance = manhatan_distance((0, 0), position_site)
            G.add_edge("s_" + str(s), "r_" + str(d), capacity=1, weight=distance)

    for d in range(len(demandes)):
        G.add_edge("r_" + str(d), "r1_" + str(d), capacity=1, weight=-K)

    for d in range(len(demandes)):
        position_site_0 = pos_sites[demandes[d]]
        for d_ in range(d+1, len(demandes)):
            position_site_1 = pos_sites[demandes[d_]]
            distance = manhatan_distance(position_site_0, position_site_1)
            G.add_edge("r1_" + str(d), "r_" + str(d_), capacity=1, weight=distance)
           
    for d in range(len(demandes)):
        G.add_edge("r1_" + str(d), "t", capacity=1, weight=0)

    for s in range(k):
        G.add_edge("s_" + str(s), "t", capacity=1, weight=0)

    mincostFlow = nx.max_flow_min_cost(G, "s", "t")

    reponse = [-1 for i in range(len(demandes))]
    for s in range(k):
        state = "s_" + str(s)
        while state != "t":
            for x in mincostFlow[state]:
                if mincostFlow[state][x] == 1:
                    state = x
                    break
    ...
    '''pour tout serveur i:
        etat = "s_i"
        tant que etat != "t":
            etat := e tel que mcf[etat][e] == 1
            si etat est de la forme "r_j":
                reponse[j] = i # Le serveur i repond à la demande j
    renvoyer reponse
    return  # ceci n'est pas une solution optimale
    '''
